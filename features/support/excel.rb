...
def compare_common_sheet(generate_report_path, benchmark_report_path)
  diff_flag = false

  generate_report = open_excel(generate_report_path)
  benchmark_report = open_excel(benchmark_report_path)
  for i in 1..(generate_report.worksheets.count) do
    sheet_name = generate_report.worksheets(i).name
    puts sheet_name
    next if sheet_name == 'DISCLAIMER'
    generate_report_sheet = generate_report.worksheets(i)
    ...
    #puts "generate_row_count:#{generate_row_count}" 
    #puts "generate_column_count:#{generate_column_count}" 

    benchmark_report_sheet = benchmark_report.worksheets(i)
    ...
    #puts "benchmark_row_count:#{benchmark_row_count}" 
